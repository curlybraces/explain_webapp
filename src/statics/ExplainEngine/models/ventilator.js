class Ventilator {
  ...
  pressureControl(p_atm) {
    if (this.TUBINGIN.pres_current >= this.pip + p_atm)
    {
      ...
      let volumeout = delta_p / (this.TUBINGIN.el_baseline * this.TUBINGIN.el_baseline_factor)
  ...
  setPEEP() {
    if (this.peep != undefined) {
      this.VENTOUT.vol_current = this.VENTOUT.vol_unstressed_baseline +  (this.peep / (this.VENTOUT.el_baseline * this.VENTOUT.el_baseline_factor))
    }
  }
  ...
  ventilatorCycling(p_atm) {
    ...
    if (this.cycling_mode === 'time') {
      ...
      if (this.insp_counter > this.t_in)
      {
        ...
        this.compliance_static = this.exhaled_tidal_volume * (this.plateau_pressure - this.peep * 1.35951)
        this.resistance_airway = (this.peak_pressure - this.plateau_pressure) / this.flow
