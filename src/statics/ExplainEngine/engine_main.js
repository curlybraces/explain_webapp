...
// define an object which is going to log the desired data
let engine_datalogger = {};
...
// realtime mode properties
let realtime_mode = false;
// state timer for the realtime mode
let realtime_timer = 0;
// realtime mode interval -> interval between graph refresh
let realtime_interval = 0.05;
// realtime mode data window -> number of seconds displayed at the same time
let realtime_datawindow = 5
// realtime vitals 
let realtime_vitals_timer = 0
let realtime_vitals_interval = 1
...
onmessage = function(e) {
  switch (e.data.type) {
    case "cmd":
      // this.console.log("ME received : ", e.data.subtype);
      if (e.data.subtype === "reset_datalogger") {
        engine_datalogger.resetDatalogger();
      }
      if (e.data.subtype === "empty_datalogger") {
        engine_datalogger.emptyDatalogger();
      }
      if (e.data.subtype === "empty_interventions") {
        engine_interventions.clearInterventions();
      }
    ...
    case "set":
      ...
      if (e.data.subtype === "config_datalogger") {
        engine_datalogger.setDataLlogger(e.data.data)
      }
      if (e.data.subtype === "config_datalogger_vitals") {
        engine_datalogger.setDataloggerVitals(e.data.data)
      }
      if (e.data.subtype === "datalogger_interval") {
        // interval of data logger updates in seconds
        if (e.data.data < current_model.modeling_stepsize) {
          engine_datalogger.interval = current_model.modeling_stepsize
        } else {
          engine_datalogger.interval = e.data.data
        }      
      }
      break;
...
const initModel = function(model_definition) {
  if (model_definition) {
    ...
    Object.keys(model_definition.avinteraction).forEach(function(key) {
      current_model.models.avinteraction[key] = model_definition.avinteraction[key];
    });
...
const calculateModel = function(time_to_calculate) {
  ...
  let no_needed_steps = parseInt(time_to_calculate / current_model.modeling_interval)

  // send starting message for this model run
  SendMessage("mes", null, null, [`current model clock at ${Math.round(current_model.model_time_total)} seconds.`]);
  SendMessage("mes", null, null, [`calculating for ${time_to_calculate}  seconds in ${no_needed_steps} steps.`]);

  // check whether the datalogger is set to empty after run
  engine_datalogger.emptyAfterRun()
  ...
  if (model_definition) 
  {
    ...
    let average_model_step_time =  total_step_execution_time / no_needed_steps
    SendMessage("mes", null, null, [`calculations ready in ${total_step_execution_time.toFixed(3)} seconds.`]);

    // send the datalog for processing to the UI
    SendMessage('data', null, null, engine_datalogger.getLoggerData())

    // send the vitals for processing to the UI
    SendMessage('vitals', null, null, engine_datalogger.getVitalsData())

    // empty the datalog as it is already copied to the UI to conserve memory
    if (engine_datalogger.datalogger_empty_after_run) {
      engine_datalogger.emptyDatalogger();
    }
  }
}
...
const startModel = function(params) {
  if (model_definition) {
    ...
    // flag that the realtime mode is switch on
    realtime_mode = true;

    realtime_datawindow = params[0]
    realtime_interval = params[1]

    // inform the datalogger to switch to realtime mode with a datawindow of x seconds
    engine_datalogger.switchRealtimeMode(true, realtime_datawindow)

  } else {
    SendMessage("mes", null, null, ["failed to start model. model not initialized!"]);
  }
};
...
const stopModel = function() {
  if (model_definition) {
    ...
    // switch off the realtime mode
    realtime_mode = false;

    // notify the datalogger to switch off the realtime mode
    engine_datalogger.switchRealtimeMode(false)
...
const modelStep = function() {
  ...
  let no_frames = Math.round(current_model["modeling_interval"] / current_model["modeling_stepsize"]);
  ...
  for (let i = 0; i < no_frames; i++) {
    ...
    engine_datalogger.modelStep(current_model.model_time_total, engine_interventions.getAnnotations())
  ...
  // if realtime mode is switch on then send datalog to UI when the realtime interval has elapsd
  if (realtime_mode){
    if (realtime_timer > realtime_interval) {
      realtime_timer = 0
      SendMessage('data_rt', null, null, engine_datalogger.getRealtimeData())
    }

    if (realtime_vitals_timer > realtime_vitals_interval) {
      realtime_vitals_timer = 0
      SendMessage('vitals', null, null, engine_datalogger.getVitalsData())
    }
  }
  
  // increase the realtime timer
  realtime_timer += current_model.modeling_interval
  realtime_vitals_timer += current_model.modeling_interval
